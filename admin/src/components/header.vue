<template>
  <header>
    <div class="header-inner">
      ...
      <div class="nav">
        <ul class="navbar">
          <li :class="shownum == 1 ? 'active': ''">
            <router-link to="/">首页</router-link>
          </li>
          <li :class="shownum == 2 ? 'active': ''">
            <router-link to="/diany">电影</router-link>
          </li>

          <li :class="shownum == 3 ? 'active': ''">
            <router-link to="/mt">影院</router-link>
          </li>
          <li :class="shownum == 4 ? 'active': ''">
            <router-link to="/mt">演出</router-link>
          </li>

          <li :class="shownum == 5 ? 'active': ''">
            <router-link to="/bangd">榜单</router-link>
          </li>
          <li :class="shownum == 6 ? 'active': ''">
            <router-link to="/redian">热点</router-link>
          </li>
          <li :class="shownum == 7 ? 'active': ''">
            <router-link to="/sc">商城</router-link>
          </li>
        </ul>
      </div>
      <div class="user-info">
        ...
        <div class="user-avatar has-login" v-if="loginState2">
          ...
          <ul class="user-menu">
            <li class="text">
              <a href="/center/likelist">喜欢列表</a>
            </li>
            <li class="text login-name" title="来瓶97年拉菲">
              <a href="/center">基本信息</a>
            </li>
            <li class="text">
              <a href="/" class="J-logout" @click="logout">退出登录</a>
            </li>
          </ul>
        </div>
      </div>
  ...
  created() {
    ...
    this.axios
      ...
      .then(res => {
        if (
          res.data.aid &&
          JSON.parse(window.localStorage.getItem("userinfo")).aid
        ) {
